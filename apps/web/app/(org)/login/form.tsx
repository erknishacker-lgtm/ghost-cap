									...
									<motion.form
										...
										onSubmit={async (e) => {
											...
											try {
												...
												setEmailSent(false);
												toast.error(
													"O login está demorando mais que o esperado. Verifique sua conexão ou extensões do navegador e tente de novo.",
												);
											} finally {
												setLoading(false);
											}
...
const LoginWithSSO = ({
	...
	return (
		...
		>
			...
			/>
			{organizationName && (
				<p className="text-sm text-gray-1">Entrando em: {organizationName}</p>
			)}
			<div>
	...
	return (
		<motion.div>
			<motion.div layout className="flex flex-col space-y-3">
				...
				>
					{loading ? "Enviando código..." : "Entrar com e-mail"}
				</MotionButton>
						...
						{oauthError && (
							<div className="flex gap-3 items-center p-3 bg-red-400 rounded-xl border border-red-600">
								...
								<p className="text-xs leading-5 text-gray-50">
									...
									login por e-mail. Digite seu e-mail.
								</p>
							</div>
						)}
